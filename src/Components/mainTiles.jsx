...
function MainTiles({ type, tileItems, vScroll }) {
	const processTiles = () => {
		...
		if (type === 'li')
			return tileItems.map((item, key) => <ListItem url={item.url} icon={type === 'project' ? 'record-vinyl' : 'user-circle'} imgSrc={item.imgSrc} text={item.name} size="md" key={key} />);
	};
	if (vScroll) {
		return <div className="d-flex flex-wrap justify-content-around align-items-start full-width">{processTiles()}</div>;
	}
	return <div className="d-flex hScroll align-items-start full-width">{processTiles()}</div>;
}
